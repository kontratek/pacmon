...
export function aiInstructionsBlock(): string {
  return [
    AI_BLOCK_START,
    `Dependency notes live in \`${NOTES_REL_PATH}\` for npm, \`${NOTES_DIR}/cargo/${NOTES_FILE_NAME}\` for Rust, and \`${NOTES_DIR}/maven/${NOTES_FILE_NAME}\` for Maven; people write below \`## <package>\`, agents under \`${AGENT_NOTES_HEADING}\`.`,
