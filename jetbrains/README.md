...
- Small inlay icons at dependency declarations in `package.json`, `Cargo.toml`, Maven `pom.xml`, `build.gradle`, `build.gradle.kts`, `mix.exs`, `build.zig.zon`, `pyproject.toml`, pip requirements files, and `go.mod` — filled when they have a note, hollow when they do not — plus an end-of-line preview and a quick-doc hover.
...
- Pure Kotlin manifest parsers; Rust, TOML, Maven, Groovy, Kotlin, Gradle, Elixir, Zig, ZON and Go plugins are not required. When optional language plugins such as ZigBrains are installed, Pacmon also registers directly for their language support.
