...
**`package.json`, `Cargo.toml`, `pom.xml`, `build.gradle` and `mix.exs` say what you depend on. Pacmon adds why.**
...
Pacmon reads a manifest as text and never runs npm, Cargo, Maven, Gradle or Mix. It sees the direct dependencies written in the file:
...
2. Open a `package.json`, `Cargo.toml`, `pom.xml`, `build.gradle`, `build.gradle.kts` or `mix.exs`. A hollow mark appears before every dependency that has no note yet.
...
VS Code 1.100 or newer, or a JetBrains IDE 2025.2 or newer. Nothing else: Pacmon reads the manifests itself, so it needs no Rust, Java, Elixir, TOML, Gradle or Mix installation, and it reads and writes files in your workspace without making network requests. In VS Code it also works in VS Code for the Web (vscode.dev, github.dev), Remote-SSH, WSL and dev containers.

## Format reference

npm notes use the `dependency-notes/1` format. Cargo, Maven, Gradle and Mix notes use `dependency-notes/2`, whose frontmatter also names the ecosystem. The reference is [`docs/format.md`](docs/format.md); the extension and the plugin follow it.
