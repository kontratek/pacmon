...
| .NET / NuGet (VS Code) | `*.csproj`, `*.fsproj`, `*.vbproj`, `Directory.Packages.props` | `.pacmon/nuget/DEPENDENCY-NOTES.md` | the case-insensitive package ID as written: `## Newtonsoft.Json` |
...
- **.NET / NuGet:** literal `PackageReference Include` items in C#, F# and VB project files; and `PackageVersion Include/Update` plus `GlobalPackageReference Include` items in `Directory.Packages.props`. Package IDs are matched case-insensitively. Imports, MSBuild expressions, project/framework references, downloads and transitive packages are not evaluated. NuGet support is currently VS Code-only.
...
npm notes use the `dependency-notes/1` format. Cargo, Maven, Gradle, Mix, Zig, Python and NuGet notes use `dependency-notes/2`, whose frontmatter also names the ecosystem. NuGet support is currently limited to VS Code; the reference is [`docs/format.md`](docs/format.md).
